...
async def handle_output_work_profile(args: ParsedArguments, request: CommandRequest):
    if args["scope"] is None:
        ...
        info_items_text = "\n".join(f"\N{BULLET} {info.full_name}: {info.value}" for info in work_info_dict.values())
    ...
    await request.event.client.send_message(request.event.message.chat_id, res_message, parse_mode="html", silent=True)
...
async def handle_output_game_profile(args: ParsedArguments, request: CommandRequest):
    if args["scope"] is None:
        ...
        info_items_text = "\n".join(f"\N{BULLET} {info.full_name}: {info.value}" for info in game_info_dict.values())
        res_message = header + info_items_text
    else:
